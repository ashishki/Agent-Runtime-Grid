...
Agent Runtime Grid is a queue-driven runtime for executing many AI or agent jobs with durable state, bounded worker execution, retries, timeouts, idempotent finalization, artifacts, logs, cost tracking, and observability. It serves AI engineers, platform engineers, and hiring reviewers who need proof that agent workloads can be operated under concurrency and failure. The control plane is deterministic: scheduling, state transitions, retry policy, budget enforcement, and audit events are code-owned, while optional AI behavior is limited to demo job payloads.
...
| Question | Answer |
|----------|--------|
| Concrete operational pain | AI and agent workflows are often run as scripts or demos that fail silently, duplicate work, lose intermediate state, and lack retry, timeout, cost, and observability semantics under concurrent load. |
...
| First user / operator who feels the pain | The portfolio owner running gdev-agent simulations and evaluation batches; secondarily an AI platform or agent infrastructure team reviewing operational skill. |
...
| Boundary | Decision |
|----------|----------|
| Work AI is expected to improve | Optional demo jobs may use AI for repo analysis, ticket classification, summarization, or synthetic agent tasks. The runtime itself does not use AI for scheduling or policy decisions. |
...
| Demo-to-production evidence required | Before trusted or remote operation, collect load-test reports, failure-injection reports, idempotency evidence, budget telemetry, and approval-boundary review for any external side-effecting job type. |
...
|----------|-----------|---------------|
| Primary shape | Hybrid decomposition | The control plane is deterministic and workflow-oriented, while individual demo jobs may contain bounded AI behavior behind explicit job-type contracts. |
| Governance level | Standard | The project is non-trivial infrastructure with persistent state, queues, workers, cost, and evidence needs, but v1 is local/demo, non-compliance, and not a privileged production autonomous runtime. |
...
|-----------------|------------------------|
| Single synchronous FastAPI wrapper | It cannot demonstrate queue lag, worker concurrency, retry isolation, cancellation, or at-least-once delivery behavior. |
...
| External scheduler as the whole project | It would hide the runtime design signal that this project is meant to demonstrate; external systems may be referenced later, not treated as the v1 proof. |
...
| Promoting from local demo to remote/cloud operation | Yes | Blast radius, secrets, auth, and recovery expectations change. |
...
| Demo job summarization, repo analysis, or ticket classification | Optional LLM inside job payload | AI behavior is isolated to job work and governed by job-type budget and tool boundaries. |
...
| Runtime mutation boundary | Runtime containers do not install packages or mutate toolchains at job execution time in v1. Demo jobs run from prebuilt images. |
...
| Default demo benchmark jobs | Stub model or deterministic fixture | The v1 reliability proof must run without paid API calls. | None required. | $0 default run. |
| Optional live LLM demo jobs | Small or standard structured-output model | The demo may classify, summarize, or analyze text, but quality demands are modest and bounded. | Stronger model only for marked jobs with approval and cost budget update. | Target below $5 per full benchmark run; per-job budget configurable. |
...
|---------|--------|---------------------|---------------|
| RAG | OFF | `docs/retrieval_eval.md` | v1 runtime does not need retrieval-backed answers. Any retrieval inside future demo jobs requires an ADR and profile activation. |
...
| Compliance | OFF | `docs/compliance_eval.md` | No named compliance framework applies to v1 synthetic/demo data. Standard security and audit controls still apply. |
...
| Job runners | `src/agent_runtime_grid/jobs/` | Stub jobs, failure injection, optional live LLM demo jobs, and artifact writers. |
...
- Mutation and inspection endpoints use local token auth when `API_TOKEN` is configured; local demo mode may disable auth only when bound to localhost.
- No real sensitive data is used in v1; demo datasets are synthetic.
...
| GitHub API | No | Optional token via environment variable | Optional repo-analysis demo; disabled unless configured. |
...
| `API_TOKEN` | No in localhost demo, yes outside local demo | API | Local token auth for non-health routes. |
...
| `GITHUB_TOKEN` | No | Optional repo-analysis jobs | Required only for GitHub API demo jobs. |
